...
class _DiseaseDetectionScreenState extends State<DiseaseDetectionScreen> {
  ...
  final _ai = GeminiDiseaseService();
  XFile? _image;
  ...
  Future<void> _pick(ImageSource source) async {
    ...
    setState(() { _image = image; _result = null; _error = null; });
  }
      ...
      body: ListView(padding: const EdgeInsets.fromLTRB(20, 12, 20, 120), children: [
        Container(
          ...
          child: Row(children: [
            ...
            Expanded(child: Text(_ai.isConfigured ? 'Gemini AI crop analysis' : 'Gemini AI ready — add your API key to enable analysis', style: const TextStyle(fontWeight: FontWeight.w800))),
          ]),
        ),
        ...
        const SizedBox(height: 14),
        ElevatedButton.icon(onPressed: _analyzing ? null : _analyze, icon: Icon(_analyzing ? Icons.hourglass_top_rounded : Icons.analytics_outlined), label: Text(_analyzing ? 'Analyzing with Gemini…' : 'Analyze with AI')),
        if (_error != null) ...[
          const SizedBox(height: 16),
          Container(padding: const EdgeInsets.all(15), decoration: BoxDecoration(color: Colors.red.withValues(alpha: .07), borderRadius: BorderRadius.circular(18), border: Border.all(color: AppColors.error.withValues(alpha: .25))), child: Row(children: [const Icon(Icons.error_outline, color: AppColors.error), const SizedBox(width: 10), Expanded(child: Text(_error!, style: const TextStyle(color: AppColors.error)))])),
        ],
        if (_result != null) ...[
          ...
          Container(padding: const EdgeInsets.all(18), decoration: BoxDecoration(color: AppColors.mint, borderRadius: BorderRadius.circular(22)), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Row(children: [const Icon(Icons.check_circle_rounded, color: AppColors.primary), const SizedBox(width: 10), Expanded(child: Text(_result!.disease, style: const TextStyle(fontWeight: FontWeight.w800, fontSize: 18))), Container(padding: const EdgeInsets.symmetric(horizontal: 9, vertical: 5), decoration: BoxDecoration(color: AppColors.primary, borderRadius: BorderRadius.circular(10)), child: Text('${_result!.confidence.toStringAsFixed(0)}%', style: const TextStyle(color: Colors.white, fontWeight: FontWeight.w800)))]),
            const SizedBox(height: 14),
            const Text('Symptoms', style: TextStyle(fontWeight: FontWeight.w800)), const SizedBox(height: 5), Text(_result!.symptoms, style: const TextStyle(color: AppColors.textSecondary, height: 1.45)),
            const SizedBox(height: 13),
            const Text('Recommended treatment', style: TextStyle(fontWeight: FontWeight.w800)), const SizedBox(height: 5), Text(_result!.treatment, style: const TextStyle(color: AppColors.textSecondary, height: 1.45)),
            const SizedBox(height: 13),
            const Text('Prevention', style: TextStyle(fontWeight: FontWeight.w800)), const SizedBox(height: 5), Text(_result!.prevention, style: const TextStyle(color: AppColors.textSecondary, height: 1.45)),
          ])),
        ],
      ]),
    );

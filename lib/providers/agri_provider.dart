import 'package:flutter/foundation.dart';
import 'package:image_picker/image_picker.dart';
import '../models/agri_models.dart';
import '../services/agri_services.dart';

class AgriProvider extends ChangeNotifier {
  final cropService = CropRecommendationService();
  final diseaseService = DiseaseDetectionService();
  final yieldService = YieldPredictionService();
  final botService = AgriBotService();
  final weatherService = WeatherService();
  final marketService = MarketService();
  final decisionService = DecisionEngineService();

  Farm farm = const Farm(
    name: 'Green Valley Farm',
    area: 4.8,
    location: 'Ahmedabad, Gujarat',
    crop: 'Wheat',
    soil: 'Loam',
    moisture: 31,
  );

  Map<String, dynamic> weather = const {
    'temperature': 29,
    'humidity': 78,
    'rainProbability': 82,
    'wind': 14,
    'uv': 6,
    'condition': 'Partly cloudy',
  };

  List<Insight> insights = const [];
  List<CropRecommendation> cropRecommendations = const [];
  DiseaseResult? disease;
  YieldResult? yieldResult;
  List<Map<String, dynamic>> marketPrices = const [];
  List<ChatMessage> messages = const [];
  bool busy = false;
  String language = 'English';

  Future<void> refresh() async {
    busy = true;
    notifyListeners();
    try {
      weather = await weatherService.getCurrent();
      insights = await decisionService.buildInsights(
        farm: farm,
        weather: weather,
      );
      marketPrices = await marketService.getPrices();
    } finally {
      busy = false;
      notifyListeners();
    }
  }

  Future<void> recommendCrop() async {
    busy = true;
    notifyListeners();
    try {
      cropRecommendations = await cropService.recommend(
        n: 82,
        p: 48,
        k: 42,
        ph: 6.7,
        temperature: 29,
        humidity: 78,
        rainfall: 185,
        soil: farm.soil,
        season: 'Kharif',
      );
    } finally {
      busy = false;
      notifyListeners();
    }
  }

  Future<void> scanDisease({required ImageSource source}) async {
    final file = await ImagePicker().pickImage(
      source: source,
      imageQuality: 80,
    );
    if (file == null) return;

    busy = true;
    notifyListeners();
    try {
      disease = await diseaseService.detect(await file.readAsBytes());
    } finally {
      busy = false;
      notifyListeners();
    }
  }

  Future<void> predictYield() async {
    busy = true;
    notifyListeners();
    try {
      yieldResult = await yieldService.predict(
        crop: farm.crop,
        area: farm.area,
        moisture: farm.moisture,
        fertilizer: 70,
        historicalYield: 4.35,
      );
    } finally {
      busy = false;
      notifyListeners();
    }
  }

  Future<void> askBot(String prompt) async {
    if (prompt.trim().isEmpty) return;
    messages = [
      ...messages,
      ChatMessage(
        text: prompt.trim(),
        isUser: true,
        time: DateTime.now(),
      ),
    ];
    notifyListeners();

    final answer = await botService.ask(prompt, language: language);
    messages = [
      ...messages,
      ChatMessage(
        text: answer,
        isUser: false,
        time: DateTime.now(),
      ),
    ];
    notifyListeners();
  }

  void setLanguage(String value) {
    language = value;
    notifyListeners();
  }
}
